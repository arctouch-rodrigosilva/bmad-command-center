...
- **API:** Use only public VS Code Extension API: `vscode.window.createTreeView`, `TreeDataProvider`, `TreeItem`, `TreeItemCollapsibleState`. No webview, no undocumented Cursor APIs.
...
- **Architecture target:** `src/views/commandCenter/commandCenterView.ts`, `commandCenterProvider.ts`, `types.ts`. Implement the TreeDataProvider in a provider file and register it from extension.ts; optional small view wrapper is fine. Align file names with architecture (camelCase for modules).
...
- **Command Center surface:** Tree View for MVP (architecture § Frontend Architecture). Use TreeDataProvider; no webview.
- **Views:** Command Center in `src/views/commandCenter/` with TreeDataProvider and types (architecture § Project Structure).
...
- [Source: _bmad-output/planning-artifacts/architecture.md] Frontend Architecture (Tree View), Project Structure (views/commandCenter), Implementation Sequence
